function Save-Credential {
    <#
		...
		.PARAMETER AccessGMSA
			The gMSA that should be able to access the credentials.
			Make sure the target computers are allowed to retrieve the password for the gMSA!
			Provide the full NTAccount name. Example: contoso\svcMyName$
		...
		.EXAMPLE
			PS C:\> Save-Credential -ComputerName DC1,DC2,DC3 -TargetCredential $cred -AccessGMSA contoso\svcDCTask$ -Path "C:\ProgramData\PowerShell\Tasks\tesk1_credential.clixml"

			Saves the credentials stored in $cred on the computers DC1, DC2, DC3 for use by the contoso\svcDCTask$ gMSA account
	#>
    ...
    process {
        $parameters = @{
            ArgumentList = $TargetCredential, $Path, $AccessAccount, $AccessGMSA
        }
        ...
        Invoke-Command @parameters -ScriptBlock {
            ...
            if ($AccessGMSA) { $accessUserName = $AccessGMSA }
            ...
            if ($accessUserName -eq "SYSTEM") { $principal = New-ScheduledTaskPrincipal -UserId $accessUserName -LogonType Interactive }
            elseif ($AccessGMSA) { $principal = New-ScheduledTaskPrincipal -UserId $AccessGMSA -LogonType Password }
            else { $principal = New-ScheduledTaskPrincipal -UserId $accessUserName -LogonType Password }
            ...
            if ($accessUserName -ne "SYSTEM" -and -not $AccessGMSA) {
