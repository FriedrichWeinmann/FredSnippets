function Get-ADGroupMemberlist {
    ...
    #>
		...
		function ConvertTo-GroupMemberMap {
			[CmdletBinding()]
			param (
				$Groups,
				$Principals
			...
			$memberships = @{}
			$identities = @{}
			foreach ($principal in $Principals) { $identities[$principal.DistinguishedName] = $principal }
			foreach ($group in $Groups) {
				$memberships[$group.DistinguishedName] = foreach ($member in $group.member) {
					$identities[$member]
				}
			}
		...
		function Get-GroupMember {
			...
			foreach ($member in $MemberMap[$Group.DistinguishedName]) {
				[PSCustomObject]@{
					...
					GroupSAM          = $RootGroup.SamAccountName
					GroupGUID         = $RootGroup.ObjectGUID
					NestedLevel       = $Depth
				}
			...
			foreach ($member in $groups) {
				Get-GroupMember -Group $member -RootGroup $RootGroup -Depth ($Depth + 1) -MemberMap $MemberMap
			}
		}
		#endregion Functions
	}
	process {
		$groups = Get-LdapObject @adParam -LdapFilter '(objectCategory=group)' -Properties ObjectGuid, SamAccountName, member, distinguishedName
		$principals = Get-LdapObject @adParam -LdapFilter '(samAccountName=*)' -Properties ObjectGuid, SamAccountName, ObjectClass, Name, DistinguishedName

		$memberships = ConvertTo-GroupMemberMap -Groups $groups -Principals $principals


		foreach ($group in $groups) {
			Get-GroupMember -Group $group -RootGroup $group -Depth 0 -MemberMap $memberships
		}
	}
}
